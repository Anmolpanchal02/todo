...
const Card = ({ data, reference, onDelete }) => {
  ...
  return (
    <motion.div
      ...
      className='relative flex-shrink-0 w-60 h-72 bg-zinc-900/90 rounded-[45px] text-white overflow-hidden flex flex-col'
    >
      {/* Title & Edit */}
        ...
        <div className='flex items-center justify-between text-lg font-bold'>
          <div className='flex items-center gap-2 overflow-hidden'>
            ...
            {isEditing ? (
              <input
                ...
                className='bg-transparent border-b text-white outline-none max-w-[120px] truncate'
              />
            ) : (
          ...
          {!isEditing && (
            <button onClick={() => setIsEditing(true)}>
              <FaEdit className='text-white hover:text-zinc-300' size={14} />
            </button>
          )}
        </div>
        ...
        <div className='text-sm mt-3 break-words whitespace-pre-wrap'>
          {isEditing ? (
            <textarea
              ...
              className='w-full bg-transparent text-white border p-1 text-sm rounded resize-none h-20'
            />
          ) : (
        ...
        <div className='flex items-center gap-4 justify-between'>
          <span className='text-sm font-semibold text-white flex items-center gap-2'>
            ...
            {!isEditing && (
              <button
                onClick={onDelete}
                className='w-6 h-6 ml-30 bg-zinc-600 hover:bg-red-500 text-white rounded-full flex items-center justify-center z-10'
                title='Delete'
              >
                <IoClose size={14} />
              </button>
      ...
      {isEditing ? (
        <div className='w-full flex justify-between p-3 gap-2 bg-zinc-700 rounded-b-[45px]'>
          <button
            onClick={handleSave}
            className='bg-green-500 px-3 py-1 text-sm text-white rounded hover:bg-green-600'
          >
            Save
          </button>
          <button
            onClick={handleCancel}
            className='bg-red-500 px-3 py-1 text-sm text-white rounded hover:bg-red-600'
          >
            Cancel
          </button>
        </div>
